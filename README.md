...
- `rssel sources` - show sources by group
  ...
  - JSON: `--json` outputs a flat list of records (group, title, url, archived, item_count). DB-only extras include `db_only: true`.
...
- File tree: `./fs/<group>/<id>-<slug>.<ext>` with a small metadata header.
...
- JSON: `--json` outputs a structured object with the same information (fields: feeds, items, last, new_window, groups, top_tags, top_sources)
